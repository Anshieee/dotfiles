...
if [[ -f /bin/brightnessctl ]]; then
	BNESS="$(brightnessctl get 'acpi_video0')"
...
case $chosen in
    "$ICON_UP")
		if [[ -f /bin/brightnessctl ]]; then
			brightnessctl -d 'acpi_video0' set +10% && $notify "Brightness Up $ICON_UP"
		elif [[ -f /usr/bin/blight ]]; then
    ...
    "$ICON_DOWN")
		if [[ -f /bin/brightnessctl ]]; then
			brightnessctl -d 'acpi_video0' set 10%- && $notify "Brightness Down $ICON_DOWN"
		elif [[ -f /usr/bin/blight ]]; then
    ...
    "$ICON_OPT")
		if [[ -f /bin/brightnessctl ]]; then
			brightnessctl -d 'acpi_video0' set 70% && $notify "Optimal Brightness $ICON_OPT"
		elif [[ -f /usr/bin/blight ]]; then
